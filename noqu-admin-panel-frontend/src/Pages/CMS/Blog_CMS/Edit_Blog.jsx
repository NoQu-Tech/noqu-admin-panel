...
const Edit_Blog = ({ refresh, userId }) => {
  ...
  const fetchNewsData = async () => {
    try {
      ...
      const response = await fetch('https://noqu.co.in/db/get-blog', {
        method: 'POST',
        headers: {
  ...
  const EditNewsfun = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://noqu.co.in/db/edit-blog', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  return (
    <div className='CMS_add-News-form'>
      <form onSubmit={EditNewsfun}>
        ...
        {thumbnail && (
          <img
            src={`https://noqu.co.in/uploads/${thumbnail}`}
            width={200}
          />
        )}
        <div className="CMS_add-News-form_row">
        ...
        {sections.map((section, index) => (
          <div key={index} className="CMS_add-News-section">
            ...
            {section.image && (
              <img
                src={`https://noqu.co.in/uploads/${section.image}`}
                alt={`Section ${index + 1}`}
                width={200}
              />
            )}
        ...
        <div className="CMS_add-News-form_button scale-hover">
          <button type="submit">{loading ? "Updating..." : "Update Blog"}</button>
        </div>
      </form>
    </div>
  )
